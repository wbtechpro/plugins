...
README
create array zoomed_info to set zooming images
[ //set images with zooming areas. legend: 
...
var zoomed_info=[
[1600,1000,2.5,'HPV-all-stack-zoom-x1600-y1000.jpg',800,800],
[1250,1280,2.5,'HPV-all-stack-zoom-x1250-y1280.jpg',800,800]
];
...
$(".zoom-pointer").click(function(){num=$(".zoom-pointer").index(this);z=$(this).attr("data-zoom");x=$(this).css("left");x=parseInt(x);y=$(this).css("top");y=parseInt(y);$(zoom_img).fadeTo("fast",.8).animate({"marginLeft":(zoom_width-z*x),"marginTop":(zoom_height-z*y),width:(2*z*zoom_width),height:(2*z*zoom_height)},{queue:false,duration:400});$(this).animate({left:zoom_width,top:zoom_height,"opacity":"hide"});$(".zoom-pointer").animate({"opacity":"hide"});setTimeout(function(){$(zoom_img).fadeTo("fast",1);$(".zoomed:eq("+num+")").animate({"opacity":"show"},{duration:400})},300);});$(".zoomed").click(function(){$(zoom_img).fadeTo("fast",1).animate({"marginLeft":0,"marginTop":0,width:(2*zoom_width),height:(2*zoom_height)},{queue:false,duration:400});$(this).animate({"opacity":"hide"},{duration:400});$(this).prev(".zoom-pointer:hidden").animate({left:x+"px",top:y+"px","opacity":"show"});$(".zoom-pointer").animate({"opacity":"show"});});});